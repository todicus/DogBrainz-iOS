...
// Sound
	...
	// pins:
	const int resetPin  = 5;  // The pin number of the reset pin. 0, 
	const int clockPin  = 4;  // The pin number of the clock pin. 3, 
	const int dataPin   = 3;  // The pin number of the data pin.  4, 
	const int busyPin   = 2;  // The pin number of the busy pin.  1, 
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if( incomingByte == 's') {
      ...
      Serial.print("new song: ");
      Serial.println(songNum);
      playSound(songNum, curVol);
    }
  }
  ...
  if(incomingScratchData.data[0] != ' ') {
    Bean.setLed(0,0,255);                           // blink blue when command received
...
// Plays song and prints out message. Not used by the "Play sound" portion of loop.
void playSound(int songNum, int volume) {
  if(serialConnected) {
